...
class MorphologyCalculator:
    ...
    def _calculate_simple_descriptors(self, mask, soma_area_um2=None):
        ...
        if props:
            ...
            # No soma outline for this cell means no soma area. This used to
            # fall back to 10% of the mask area, which put a fabricated number
            # in a column that otherwise holds a measurement, with nothing
            # marking which rows were which -- and mmps_phenotype_classifier.R
            # divides this by mask_area, so every fallback row fed the
            # classifier a soma_cell_ratio of exactly 0.1. Blank reads as NA.
            params['soma_area'] = (soma_area_um2 if soma_area_um2 is not None
                                   else '')
...
Computes per-mask morphology metrics from MMPS-exported mask TIFF files:
  ...
  - soma_area (from somas/ folder; blank when there is no outline for a cell)
...
def compute_metrics(mask_path, pixel_size, soma_area_um2=None):
    ...
    # Soma area. Blank when the somas/ folder has no outline for this cell:
    # 10% of the mask area was a fabricated stand-in, and anything dividing it
    # by mask_area got exactly 0.1 for every such row. Mirrors MMPSv2.12.py.
    params['soma_area'] = (round(soma_area_um2, 4)
                           if soma_area_um2 is not None else '')
...
<span style='color: #00B400;'><b>✅ Mask Characteristics Processed</b></span> - Morphological analysis complete
    ...
    def _on_morph_finished(self, all_results):
        ...
        self._classify_and_log_morphotypes(all_results)
    ...
    def _classify_and_log_morphotypes(self, all_results):
