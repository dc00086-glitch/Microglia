...
class MorphologyCalculator:
    ...
    def _calculate_polarity(self, coords, centroid):
        """Calculate axis extents from PCA on mask pixel coordinates."""
        params = {}
        centered = coords - centroid
...
'''
    ...
    def calculate_colocalization(self, mask, img_name):
        """Colocalization is determined by soma_group during multi-pass picking."""
        return {}
...
<span style='color: #00B400;'><b>✅ Mask Characteristics Processed</b></span> - Morphological analysis complete
    ...
    def _on_morph_finished(self, all_results):
        ...
        if self.colocalization_mode:
            ...
            for result in all_results:
                group = result.get('soma_group', '')
                result['is_colocalized'] = group == 'coloc'
                if group == 'coloc':
                    coloc_count += 1
                elif group == 'single_channel':
                    single_count += 1
            self.log(f"✓ Colocalization: {coloc_count} colocalized, {single_count} single-channel")
    ...
    def open_image_labeling(self):
        ...
        # Instructions
        info_label = QLabel(
            "<b>Enter Animal ID, Treatment, and Day for each selected image:</b><br>"
        ...
        table.setHorizontalHeaderLabels(["Image Name", "Animal ID", "Treatment", "Day"])
        ...
        for row, img_name in enumerate(sorted(selected_images)):
            ...
            day_item = QTableWidgetItem(self.images[img_name].get('day', ''))
            table.setItem(row, 3, day_item)
    ...
    def collect_metadata_for_images(self):
        ...
        for img_name in selected_images:
            ...
            day_input = QLineEdit()
            day_input.setPlaceholderText("e.g., Day 1, Day 7, P30")
            day_input.setText(self.images[img_name].get('day', ''))

            img_layout.addRow("Animal ID:", animal_id_input)
            img_layout.addRow("Treatment:", treatment_input)
            img_layout.addRow("Day:", day_input)
        ...
        def save_metadata():
            for img_name, inputs in image_inputs.items():
                ...
                self.images[img_name]['day'] = inputs['day'].text().strip()
            dialog.accept()
    ...
    def _save_batch_results(self, results):
        ...
        for key in ['soma_id', 'image_name', 'animal_id', 'treatment', 'day', 'soma_idx', 'soma_group']:
            if key in keys:
                keys.remove(key)

        # Separate colocalization keys from morphology keys for better organization
        coloc_keys = ['is_colocalized']
        ...
        id_keys = ['image_name', 'animal_id', 'treatment', 'day']
        if has_groups:
        ...
        with open(metadata_path, 'w', newline='') as f:
            ...
            writer.writerow(['mask_filename', 'soma_filename', 'image_name', 'soma_id', 'soma_idx',
                             ...
                             'major_axis_um', 'minor_axis_um', 'animal_id', 'treatment', 'day'])
