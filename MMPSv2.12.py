...
class MicrogliaAnalysisGUI(QMainWindow):
    ...
    def _open_imagej_cluster_dialog(self):
        ...
        self.cluster_module_load = QLineEdit("export JAVA_HOME=$SCRATCH/jdk-21 && export PATH=$SCRATCH/jdk-21/bin:$PATH")
        self.cluster_module_load.setToolTip("SNT (Sholl analysis) requires Java 21+. Points to OpenJDK 21 in $SCRATCH.")
...
MERGE_JOB_ID=$(sbatch --parsable \\
    ...
    --wrap="{module_line}
python3 \\"$SCRIPT_DIR/mmps_spread_analysis.py\\" \\"$MMPS_OUTPUT_DIR\\" --merge-only"
)
