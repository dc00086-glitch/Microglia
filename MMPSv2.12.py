...
# 3D Z-stack analysis has been removed; MMPS is 2D-only.
_HAS_3D = False
...
def _growth_intensity_floor(roi, cy_roi, cx_roi, floor_mode,
    ...
    ``floor_mode``:
      'percent'      — floor = ``min_intensity_percent`` % of the image max,
                       computed EXACTLY as the Intensity Threshold Preview so the
                       preview matches the masks (set % to 0 for no floor). An
                       optional local-max per-pixel floor is added when a local
                       window is set.
    ...
    # Default 'percent' mode — floor = percent of the image max, IDENTICAL to the
    # Intensity Threshold Preview (threshold = img_max * percent/100). Use the
    # whole-image ``global_max`` when provided so every soma shares the same floor
    # the preview shows; fall back to the ROI max otherwise.
    intensity_floor = 0.0
    intensity_floor_map = None
    if min_intensity_percent and min_intensity_percent > 0:
        base = float(global_max) if (global_max is not None and global_max > 0) \
            else float(roi.max())
        if base > 0:
            intensity_floor = base * (min_intensity_percent / 100.0)
...
class MicrogliaAnalysisGUI(QMainWindow):
    ...
    def init_ui(self):
        ...
        r_script_action = export_menu.addAction("Generate R Analysis Script...")
        r_script_action.setToolTip("Generate an R script with ggplot2 plots for your morphology results")
        r_script_action.triggered.connect(self._export_r_script)
...
def create_competitive_masks(processed_img, soma_outlines_data, area_list_um2,
    ...
    if use_min_intensity and min_intensity_percent > 0:
        # Floor = percent of the image max, identical to the Intensity Threshold
        # Preview so the preview matches the masks (0% = no floor).
        img_max = float(processed_img.max())
        if img_max > 0:
    ...
    def import_imagej_results(self):
        ...
        if not morphology_rows:
            ...
            for row in morphology_rows:
                ...
                morph_area_str = row.get('target_area_um2', row.get('area_um2', ''))
                try:
                    morph_area = int(float(morph_area_str)) if morph_area_str else None
                except (ValueError, TypeError):
                    morph_area = None

                # Try exact match (image, soma, area) first
                # Then try without area (for ImageJ data that didn't embed area)
                key_exact = (img_name, soma_id, morph_area)
                ...
                for lookup, counter_name in [
                    ...
                    if not lookup:
                        continue
                    match_data = lookup.get(key_exact) or lookup.get(key_no_area)
                    if match_data:
...
<span style='color: #00B400;'><b>✅ Mask Characteristics Processed</b></span> - Morphological analysis complete
    ...
    def _create_competitive_masks(self, processed_img, soma_outlines_data, area_list_um2,
        ...
        # Intensity floor = percent of the image max, identical to the Intensity
        # Threshold Preview so the preview matches the masks (0% = no floor).
        intensity_floor = 0.0
        intensity_floor_map = None
    ...
    def _compute_nearest_neighbor_distances(self, all_results):
        ...
        Coordinates are parsed directly from soma_id (format: soma_Y_X)
        so this works regardless of whether img_data['somas'] is loaded.
        """
        import re as _re_nnd

        # Group by image, deduplicate somas, parse coords from soma_id
        by_image = {}
        for i, result in enumerate(all_results):
            img_name = result.get('image_name', '')
            if img_name not in by_image:
                by_image[img_name] = {'result_indices': [], 'soma_coords': {}}
            by_image[img_name]['result_indices'].append(i)

            soma_id = result.get('soma_id', '')
            if soma_id and soma_id not in by_image[img_name]['soma_coords']:
                m = _re_nnd.match(r'soma_(\d+)_(\d+)', soma_id)
                if m:
                    by_image[img_name]['soma_coords'][soma_id] = (float(m.group(1)), float(m.group(2)))

        computed = 0
        for img_name_base, img_info in by_image.items():
            soma_coords = img_info['soma_coords']
            ...
            # Get pixel size
            full_img_name = next((fn for fn in self.images if os.path.splitext(fn)[0] == img_name_base), None)
            px_x, px_y = self._get_pixel_size_xy(full_img_name)

            # Compute NND for each unique soma
            soma_nnd = {}
            sids = list(soma_coords.keys())
            coords_list = [soma_coords[s] for s in sids]
            ...
            for j in range(len(sids)):
                ...
                for k in range(len(sids)):
                    ...
                    if dist < min_dist:
                        min_dist = dist
                soma_nnd[sids[j]] = round(min_dist, 2)

            for idx in img_info['result_indices']:
                soma_id = all_results[idx].get('soma_id', '')
                nnd = soma_nnd.get(soma_id, 0.0)
                all_results[idx]['nearest_neighbor_um'] = nnd
                if nnd > 0:
                    computed += 1
    ...
    def _export_r_script(self):
        """Generate an R script with ggplot2 plots for morphology results."""
        csv_path = None
        if self.output_dir:
            candidate = os.path.join(self.output_dir, "combined_morphology_results.csv")
            if os.path.isfile(candidate):
                csv_path = candidate

        if csv_path is None:
            csv_path, _ = QFileDialog.getOpenFileName(
                self, "Select Morphology Results CSV", "",
                "CSV Files (*.csv);;All Files (*)")
            if not csv_path:
                return

        # Read CSV to discover columns
        import csv as _csv
        with open(csv_path, 'r') as f:
            reader = _csv.DictReader(f)
            columns = list(reader.fieldnames)

        # Detect available morphology columns
        morph_cols = []
        candidate_cols = [
        ...
        has_treatment = 'treatment' in columns
        has_animal = 'animal_id' in columns
        has_group = 'soma_group' in columns
        group_var = 'treatment' if has_treatment else 'image_name'

        # Detect coloc columns
        coloc_cols = [c for c in columns if c.startswith('coloc_') or c.startswith('ch') or c == 'pearson_r'
                      or c in ('n_coloc_pixels', 'ch1_coloc_percent', 'ch2_coloc_percent',
                               'n_ch1_signal', 'n_ch2_signal')]
        coloc_cols = [c for c in coloc_cols if c in columns]

        # Build the R script
        script = f'''# ============================================================================
# MMPS Morphology Analysis — R Script
# Generated by MMPSv2 on {time.strftime("%Y-%m-%d %H:%M:%S")}
#
# Input: {os.path.basename(csv_path)}
# ============================================================================
...
data <- read.csv("{os.path.basename(csv_path)}", stringsAsFactors = FALSE)
...
'''
        for i, col in enumerate(morph_cols):
            comma = ',' if i < len(morph_cols) - 1 else ''
            script += f'    {col}_mean = mean({col}, na.rm = TRUE),\n'
            script += f'    {col}_sd = sd({col}, na.rm = TRUE),\n'
            script += f'    {col}_sem = sd({col}, na.rm = TRUE) / sqrt(n()){comma}\n'
...
# Theme for publication-ready figures
...
'''
        for col in morph_cols:
            ...
            script += f'''
p_{col} <- ggplot(data, aes(x = {group_var}, y = {col}, fill = {group_var})) +
  geom_boxplot(outlier.shape = NA, alpha = 0.7) +
  geom_jitter(width = 0.2, size = 1, alpha = 0.4) +
  labs(title = "{nice_name}", y = "{nice_name}{unit}", x = "") +
  theme_pub +
...
# --- Combined panel figure ---
plot_list <- list('''
        script += ', '.join(f'p_{col}' for col in morph_cols[:6])
        script += ''')
combined <- ggarrange(plotlist = plot_list, ncol = 3, nrow = 2,
                      common.legend = TRUE, legend = "top")
ggsave("combined_morphology_plots.png", combined, width = 18, height = 12, dpi = 300)
'''

        # Correlation matrix
        if len(morph_cols) >= 3:
            cols_str = ', '.join(f'"{c}"' for c in morph_cols)
            script += f'''
# --- Correlation matrix ---
if (!require("corrplot", quietly = TRUE)) install.packages("corrplot")
library(corrplot)

morph_numeric <- data[, c({cols_str})]
morph_numeric <- morph_numeric[complete.cases(morph_numeric), ]
cor_matrix <- cor(morph_numeric)

png("correlation_matrix.png", width = 10, height = 10, units = "in", res = 300)
corrplot(cor_matrix, method = "color", type = "upper",
         addCoef.col = "black", number.cex = 0.7,
         tl.col = "black", tl.cex = 0.9,
         title = "Morphology Correlation Matrix",
         mar = c(0, 0, 2, 0))
dev.off()
'''
...
# Load colocalization summary if available
coloc_summary_file <- "colocalization_summary.csv"
if (file.exists(coloc_summary_file)) {
  coloc_summary <- read.csv(coloc_summary_file, stringsAsFactors = FALSE)
  ...
  # Stacked bar chart: coloc vs single-channel per image
  coloc_long <- coloc_summary %>%
    select(image_name, treatment, matches("_only$|\\\\+")) %>%
    pivot_longer(cols = -c(image_name, treatment),
                 names_to = "cell_type", values_to = "count")

  p_stacked <- ggplot(coloc_long, aes(x = image_name, y = count, fill = cell_type)) +
    geom_bar(stat = "identity", position = "stack") +
    labs(title = "Cell Counts by Image", y = "Number of Cells", x = "") +
    theme_pub +
    theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 8))
  ggsave("plot_cell_counts_stacked.png", p_stacked, width = 12, height = 6, dpi = 300)
}
'''

        if has_treatment and has_animal:
            script += f'''
# --- Per-animal summary (for nested/repeated measures) ---
animal_summary <- data %>%
  group_by(animal_id, {group_var}) %>%
  summarise(
    n_cells = n(),
'''
            for i, col in enumerate(morph_cols[:6]):
                comma = ',' if i < min(len(morph_cols), 6) - 1 else ''
                script += f'    {col}_mean = mean({col}, na.rm = TRUE){comma}\n'
            script += '''    , .groups = "drop"
  )
write.csv(animal_summary, "per_animal_summary.csv", row.names = FALSE)
cat("Per-animal summary saved\\n")
'''

        script += '''
cat("\\n============================================\\n")
cat("All plots saved to current directory\\n")
cat("============================================\\n")
'''
        ...
        save_path, _ = QFileDialog.getSaveFileName(
            self, "Save R Script",
            os.path.join(self.output_dir or "", "mmps_analysis.R"),
        ...
        self.log(f"R script saved to: {save_path}")
        self.log(f"  Metrics included: {', '.join(morph_cols)}")
        if coloc_cols:
            self.log(f"  Colocalization plots included")
        self.log(f"  To run: open in RStudio or run 'Rscript {os.path.basename(save_path)}'")

        QMessageBox.information(self, "R Script Generated",
            f"R script saved to:\n{save_path}\n\n"
            f"Metrics: {len(morph_cols)}\n"
            f"Plots: box plots, combined panel, correlation matrix"
            f"{', colocalization' if coloc_cols else ''}\n\n"
            f"To run:\n"
            f"  1. Copy the CSV to the same folder as the script\n"
            f"  2. Open in RStudio, or run: Rscript {os.path.basename(save_path)}")

    # ----------------------------------------------------------------
    # EXPORT: FIGURE-READY COMPOSITES
    # ----------------------------------------------------------------
    ...
    def _export_figure_composites(self):
        ...
        outline_rgb = color_names[outline_color_combo.currentText()]
        ...
        def _draw_masks_on(img_rgb, masks_to_draw, img_name_str, w):
            for mask_data in masks_to_draw:
                mask = mask_data.get('mask')
                if mask is None:
                    self._reload_mask_from_disk(mask_data, img_name)
                    mask = mask_data.get('mask')
                if mask is None:
                    continue
