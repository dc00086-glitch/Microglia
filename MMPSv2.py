...
class InteractiveImageLabel(QLabel):
    ...
    def set_zoom(self, level, center=None):
        """Set zoom to a specific level, optionally centered on a point in image coords (row, col)"""
        self.zoom_level = max(self.min_zoom, min(self.max_zoom, level))
        if center is not None and self.pix_source:
            img_h, img_w = self.pix_source.height(), self.pix_source.width()
            self.view_center_x = center[1] / img_w
            self.view_center_y = center[0] / img_h
        self._update_display()
    ...
    def mousePressEvent(self, event):
        ...
        if event.button() == Qt.MiddleButton and self.zoom_level > 1.0:
            self._panning = True
    ...
    def mouseMoveEvent(self, event):
        """Handle mouse move for panning and point dragging"""
        # Handle middle-click panning
        if self._panning and self._pan_start and self.pix_source and self.scaled_pixmap:
            delta = event.pos() - self._pan_start
            self._pan_start = event.pos()
            pixmap_w = self.scaled_pixmap.width()
            pixmap_h = self.scaled_pixmap.height()
            # Convert pixel delta to normalized view center shift
            if pixmap_w > 0 and pixmap_h > 0:
                self.view_center_x -= delta.x() / pixmap_w
                self.view_center_y -= delta.y() / pixmap_h
                self.view_center_x = max(0.0, min(1.0, self.view_center_x))
                self.view_center_y = max(0.0, min(1.0, self.view_center_y))
                self._update_display()
            return
    ...
    def mouseReleaseEvent(self, event):
        """Handle mouse release to finish dragging or panning"""
...
<span style='color: #00B400;'><b>✅ Mask Characteristics Processed</b></span> - Morphological analysis complete
    ...
    def add_polygon_point(self, coords):
        ...
        if queue_idx < len(self.outlining_queue):
            ...
            soma = img_data['somas'][soma_idx]
            soma_id = img_data['soma_ids'][soma_idx]
            # Save current zoom state before set_image
            saved_zoom = self.processed_label.zoom_level
            saved_cx = self.processed_label.view_center_x
            saved_cy = self.processed_label.view_center_y
            pixmap = self._get_outlining_pixmap(img_data)
            self.processed_label.set_image(pixmap, centroids=[soma], polygon_pts=self.polygon_points)
            # Restore zoom state
            if saved_zoom > 1.0:
                self.processed_label.zoom_level = saved_zoom
                self.processed_label.view_center_x = saved_cx
                self.processed_label.view_center_y = saved_cy
                self.processed_label._update_display()
            self.nav_status_label.setText(
                f"Soma {queue_idx + 1}/{len(self.outlining_queue)} | "
                f"Image: {img_name} | ID: {soma_id} | "
    ...
    def undo_last_polygon_point(self):
        ...
        if len(self.polygon_points) > 0:
            ...
            # Refresh the display while preserving zoom
            queue_idx = len([data for img_data in self.images.values() for data in img_data['soma_outlines']])
            if queue_idx < len(self.outlining_queue):
                img_name, soma_idx = self.outlining_queue[queue_idx]
                img_data = self.images[img_name]
    ...
    def _show_current_mask(self):
        ...
        # Update current image so _to_image_coords works for scroll zoom
        self.current_image_name = img_name
        ...
        # Autozoom onto the soma for easier mask review
        ...
        zoom_text = f" | Zoom: {self.mask_label.zoom_level:.1f}x"

        self.nav_status_label.setText(
            f"Mask {self.mask_qa_idx + 1}/{len(self.all_masks_flat)} | "
            f"{img_name} | {mask_data['soma_id']} | "
            f"Area: {mask_data['area_um2']} µm² | {status_text}{zoom_text}"
        )
